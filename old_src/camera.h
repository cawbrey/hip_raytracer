...
__device__ vec3 random_in_unit_disk(hiprandState *local_rand_state) {
    vec3 p;
    do {
        p = 2.0f * vec3(hiprand_uniform(local_rand_state),
                        hiprand_uniform(local_rand_state), 0) -
            vec3(1, 1, 0);
    } while (dot(p, p) >= 1.0f);
    return p;
}
...
public:
  ...
  __device__ ray3 get_ray(float s, float t,
                          hiprandState *local_rand_state) const {
        vec3 rd = lens_radius * random_in_unit_disk(local_rand_state);
        vec3 offset = u * rd.x + v * rd.y;
        return ray3({
            origin + offset, lower_left_corner + s * horizontal +
