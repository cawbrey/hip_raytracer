...
public:
    ...
    __device__ BoundingBox(const float2 &x, const float2 &y, const float2 &z) : x(x), y(y), z(z) {}
    ...
    __device__ float lengthX() const {
        return x.y - x.x;
    }

    __device__ float lengthY() const {
        return y.y - y.x;
    }

    __device__ float lengthZ() const {
        return z.y - z.x;
    }

    __device__ float minX() const {
        return x.x;
    }

    __device__ float minY() const {
        return y.x;
    }

    __device__ float minZ() const {
        return z.x;
    }

    __device__ float maxX() const {
        return x.y;
    }

    __device__ float maxY() const {
        return y.y;
    }

    __device__ float maxZ() const {
        return z.y;
    }
};
