...
__device__ Color color_ray(const ray3 &r, Hittable **world,
    ...
    for (int i = 0; i < 50; i++) {
        HitRecord rec{};
        if (!(*world)->hit(cur_ray, 0.001f, MAXFLOAT, rec)) break;
        ...
        if (rec.mat_ptr->scatter(cur_ray, rec, tempAtt, scattered,
                                 local_rand_state)) {
...
__global__ void render(vec3 *fb, uint max_x, uint max_y, uint ns, camera **cam,
...
__global__ void create_world(Hittable **d_list, Hittable **d_world,
    ...
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        ...
        Material *checkerBoard = new TexturedLambertion(
            new CheckerBoardTexture({1, 0, 1}, {0, 0, 0}, 10.0));
...
int main() {
    ...
    Hittable **d_objects;
    checkHipErrors(
        hipMalloc((void **) &d_objects, NUM_OBJECTS * sizeof(Hittable *)));
    Hittable **d_world;
    ...
    checkHipErrors(hipGetLastError());
    checkHipErrors(hipDeviceSynchronize());
    render<<<blocks, threads>>>(fb, width, height, numSamples, d_camera, d_world,
                                d_rand_state);
    checkHipErrors(hipGetLastError());
    checkHipErrors(hipDeviceSynchronize());
    stop = clock();
