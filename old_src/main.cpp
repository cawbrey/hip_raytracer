...
__global__ void
render(vec3 *fb, uint max_x, uint max_y, uint ns, camera **cam, Hittable **world, hiprandState *rand_state) {
...
__global__ void
create_world(Hittable **d_list, Hittable **d_world, camera **d_camera, int nx, int ny, Color *d_earth_img) {
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        // Create Materials
        ...
        Material *checkerBoard = new TexturedLambertion(new CheckerBoardTexture({1, 0, 1}, {0,0,0}, 10.0));
        Material *earth = new TexturedLambertion(new ImageTexture(new Image::Bitmap(d_earth_img, 2000, 1000)));
...
int main() {
    ...
    std::cerr << "Rendering a " << width << "x" << height << " image with " << numSamples << " samples per pixel ";
    ...
    thrust::device_ptr<Color> d_earth_img = thrust::device_malloc<Color>(h_earth_img->width * h_earth_img->height);
    ...
    // build the world
    create_world<<<1, 1>>>(d_objects, d_world, d_camera,width, height, thrust::raw_pointer_cast(d_earth_img));
    checkHipErrors(hipGetLastError());
    checkHipErrors(hipDeviceSynchronize());
