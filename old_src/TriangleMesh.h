...
public:
    ...
    __device__ TriangleMesh(point3 *vertices, uint numVertices, Transform transform, Material *mat): faces() , meshHead(nullptr)
    {
        mat_ptr = mat;
        faces = new Hittable*[numVertices / 3];

        for (int i = 0; i < numVertices; i += 3) {
            faces[i] = new Triangle(vertices[i], vertices[i + 1], vertices[i + 2], mat);
        }

        //faces.push_back(std::make_shared<Triangle>(vertices[0], vertices[1], vertices[2], mat));
        meshHead = new Bvh(faces, 0, numVertices / 3);

        boundingBox = meshHead->boundingBox;
    }

    __device__ virtual bool hit(const ray3 &r, float t_min, float t_max, HitRecord &rec) const override {
