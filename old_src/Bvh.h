...
public:
    __device__ Bvh(Hittable **objects, int64_t start, int64_t end) {
        for (size_t object_index = start; object_index < end; object_index++)
            boundingBox =
                    BoundingBox(boundingBox, objects[object_index]->boundingBox);
        ...
        if (object_span == 1) {
            ...
            auto comparator =
                    boundingBox.lengthX() > boundingBox.lengthY()
                        ? (boundingBox.lengthX() > boundingBox.lengthZ()
                               ? box_x_compare
                               : box_z_compare)
                        : (boundingBox.lengthY() > boundingBox.lengthZ()
                               ? box_y_compare
                               : box_z_compare);
  ...
  __device__ bool hit(const ray3 &r, float tmin, float tmax,
        ...
        if (!boundingBox.hit(r, tmin, tmax)) return false;

        bool hit_left = leftChild && leftChild->hit(r, tmin, tmax, rec);
        bool hit_right =
                rightChild && rightChild->hit(r, tmin, hit_left ? rec.t : tmax, rec);

        return hit_left || hit_right;
    }

private:
  __device__ static inline bool box_x_compare(const Hittable *a,
                                              const Hittable *b) {
        return a->boundingBox.minX() < b->boundingBox.minX();
    }

  __device__ static inline bool box_y_compare(const Hittable *a,
                                              const Hittable *b) {
        return a->boundingBox.minY() < b->boundingBox.minY();
    }

  __device__ static inline bool box_z_compare(const Hittable *a,
                                              const Hittable *b) {
        return a->boundingBox.minZ() < b->boundingBox.minZ();
    }

  __device__ static void shittySorter(Hittable **objects, int64_t start,
                                      int64_t end,
                                      bool (*comparator)(const Hittable *a,
                                                         const Hittable *b)) {
        ...
        for (int64_t i = start; i < end - 1; i++) {
            ...
            for (int64_t j = i + 1; j < end; j++) {
                // Compare current object with the object at min_index using the
                // comparator
