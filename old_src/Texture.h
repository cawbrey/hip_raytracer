...
public:
  ...
  __device__ Color value(float u, float v) const override { return color; }
};
...
public:
    __device__ explicit CheckerBoardTexture(Color c1, Color c2, float frequency)
        : frequency(frequency), color1(c1), color2(c2) {
    };

  __device__ Color value(float u, float v) const override {
        return ((uint)(v * frequency) + (uint)(u * frequency)) % 2 == 0
                   ? color1
                   : color2;
    }
};
